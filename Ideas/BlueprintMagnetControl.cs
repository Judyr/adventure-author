...
namespace AdventureAuthor.Ideas
{
	...
	{
		private INWN2Blueprint blueprint;
		public INWN2Blueprint Blueprint {
			get { return blueprint; }
		}
		
		
		public BlueprintMagnetControl(INWN2Blueprint blueprint) : base()
		{
			Background = new System.Windows.Media.LinearGradientBrush(System.Windows.Media.Colors.Green,
			                                                          System.Windows.Media.Colors.Red,45);
			...
			if (blueprint is NWN2EncounterBlueprint) {
				...
				resrefOrTag = encounter.Tag;
			}
			else if (blueprint is NWN2StoreBlueprint) {
				NWN2StoreBlueprint store = (NWN2StoreBlueprint)blueprint;
				resrefOrTag = store.Tag;
			}
			else {
				resrefOrTag = blueprint.TemplateResRef.Value;
			}
			textBlock.Text = resrefOrTag;
			textBlock.FontStyle = FontStyles.Italic;
			textBlock.ToolTip = "This is the resref - a special name\n" +
				"that uniquely identifies this particular blueprint.";
			textBlock.Margin = new Thickness(5);
			textBlock.HorizontalAlignment = HorizontalAlignment.Center;
			//magnetMainStackPanel.Children.Insert(0,textBlock);
			magnetMainStackPanel.Children.Add(textBlock);
									
			string text = GetTextForBlueprintMagnet(blueprint);
			Idea = new Idea(text,IdeaCategory.Toolset,User.GetCurrentUserName(),DateTime.Now);
